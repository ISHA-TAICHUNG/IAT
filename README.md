...
## 反饋處理：用 Google Sheet 修正題目（不需碰程式碼）

使用者在作答中或成績頁點「💬 反饋」時，資料會自動寫入 **「反饋紀錄」** 工作表。

### 首次設定修正面板

1. 打開你的 Google Sheet（有反饋紀錄那份）
2. 點上方選單 **📚 題庫管理 → 📋 建立修正指令分頁**
3. 會自動建立「修正指令」分頁和「修正指令說明」分頁
...
到「反饋紀錄」分頁，看使用者回報了什麼問題，記下 **職類** 和 **題目ID**。
...
系統會自動：
- ✅ 修改 Google Drive 上的 JSON 檔
- ✅ 同步更新 categories.json 的題數
- ✅ 清除快取，修正立即生效
- ✅ 在「執行狀態」欄標記處理結果

#### 步驟 4：標記反饋已處理

回到「反饋紀錄」分頁，將對應的「已處理」欄改為 `是`。
