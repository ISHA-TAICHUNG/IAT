...
```
exam-site/
├── index.html               首頁（選職類 + 進站須知 + 測驗前須知 + 歷史成績）
├── exam.html                作答頁（倒計時 + 提前結束 Modal + 反饋 Modal）
...
├── js/
│   ├── config.js            GAS URL、計分、時間限制等設定
│   ├── utils.js             共用工具（fetchWithTimeout、shuffleOptions、歷史管理）
│   ├── app.js               首頁邏輯（下拉選單、須知彈窗、翻譯、歷史顯示）
│   ├── exam.js              作答核心（選題、計分、倒計時、反饋、錯題複習、鍵盤）
