...
- 🗂 27 個職類（業務主管 / 作業主管 / 職護 / 即測即評）
- 🌏 即測即評職類自動顯示國旗與母語翻譯（印尼、越南、菲律賓、泰國）
- 🎲 每次從題庫隨機抽題，選項順序也隨機排列
...
使用者在作答中或成績頁點「💬 反饋」時，資料會自動寫入 **「反饋紀錄」** 工作表（含題目文字、四個選項、預設答案）。
同一題目相同類型的反饋會自動去重（合併為次數 +1）。

1. 查看「反饋紀錄」分頁，記下 **職類**、**題目ID** 和 **預設答案**（系統原本的正確答案）
2. 到「修正指令」分頁填寫修正：
...
```html
<!-- 把 ?v=20260222c 改成新日期 -->
<link rel="stylesheet" href="css/style.css?v=20260222c" />
<script src="js/app.js?v=20260222c"></script>
```
