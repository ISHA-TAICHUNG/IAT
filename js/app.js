...
async function loadCategories() {
  var res = await fetchWithTimeout(CONFIG.GAS_URL + "?action=categories&token=" + encodeURIComponent(CONFIG.API_TOKEN) + "&clientId=" + encodeURIComponent(getOrCreateClientId()));
...
function renderDropdown(cats) {
  ...
  if (lang !== 'zh-TW') {
    ...
    // 中文模式：分群組
    var groupOrder = ["全國技術士", "業務主管", "作業主管", "職護", "即測即評"];
