...
document.getElementById("result-area").innerHTML = `
  <div class="result-card">
    ...
    <h2 style="margin-bottom:8px;font-size:1rem;">${escapeHtml(catName)}</h2>
    <div class="stats-row">
    ...
    <div class="result-actions">
      ...
      <a href="exam.html?cat=${encodeURIComponent(catId)}&examMode=${examMode}" class="btn btn-primary">🔄 重新練習</a>
  ...
  <div class="wrong-list">
    ...
    ${wrongList.map(item => `
      <div class="wrong-item">
        ...
        <div class="wi-q">${escapeHtml(item.q.q)}</div>
        <div class="wi-meta">
          ${item.ans.chosen !== null && item.ans.chosen !== item.q.answer
    ? `<span class="wi-your">你的答案：${escapeHtml(item.q.options[item.ans.chosen])}</span>`
    : ""}
          <span class="wi-correct">正確答案：${escapeHtml(item.q.options[item.q.answer])}</span>
        </div>
